...
class MCTS(object):
    def __init__(self, policy_value_fn, c_puct=5, n_playout=10000, disp=False):
        ...
        self.As_prev = {}
        # key: hash(state)
        # value: action took previously

        self.Qsa_all = {}
        # key: tuple(int hash(numpy-state), int action)
        # value: a list of float q values
    ...
    def _playout(self, state):
        ...
        cur_best = -1000000
        best_action = None
        a_prev = (None if board_hash not in self.As_prev else self.As_prev[board_hash])

        # the bound of reward should be [0, 1]
        # but the reward here is -1 and 1
        # so we compensate it by adding a Nsa
        if a_prev:
            p_prev = (self.Qsa[(board_hash, a_prev)] + self.Nsa[(board_hash, a_prev)]) / (2 * self.Nsa[(board_hash, a_prev)])
        else:
            p_prev = None

        for a in valid_moves:
            bound = math.log(self.Ns[board_hash] + self.EPS, 2)
            if (board_hash, a) in self.Qsa and p_prev:
                p_cur = (self.Qsa[(board_hash, a)] + self.Nsa[(board_hash, a)]) / (2 * self.Nsa[(board_hash, a)])
                klc = self.Nsa[(board_hash, a)] * kl_divergence(p_cur, p_prev)
                u = self.Qsa[(board_hash, a)] + self.c_puct * self.Ps[board_hash][a] * min(klc, bound)
            else:
                u = self.c_puct * self.Ps[board_hash][a] * bound
        ...
        if (board_hash, best_action) in self.Qsa:
            ...
            self.Qsa[(board_hash, best_action)] = (self.Qsa[(board_hash, best_action)] * n_old + value) \
                                                  / (1 + n_old)
            # for UCB1 tuned
            self.Qsa_all[(board_hash, best_action)].append(self.Qsa[(board_hash, best_action)])
            self.Nsa[(board_hash, best_action)] = n_old + 1.0
        else:
            # first time
            self.Qsa[(board_hash, best_action)] = value
            # for UCB1 tuned
            self.Qsa_all[(board_hash, best_action)] = [value]
