    ...
    def _playout(self, state):
        ...
        for a in valid_moves:
            if (board_hash, a) in self.Qsa:
                var = np.var(self.Qsa_all[(board_hash, a)])
                v = var + math.sqrt(2 * math.log(self.Ns[board_hash] + self.EPS, 2) / self.Nsa[(board_hash, a)])
                u = self.Qsa[(board_hash, a)] + self.c_puct * self.Ps[board_hash][a] * \
                        math.sqrt(min(v, 0.25) * math.log(self.Ns[board_hash] + self.EPS, 2) / self.Nsa[(board_hash, a)])
            else:
                u = 1.0 * self.c_puct * self.Ps[board_hash][a] * math.sqrt(math.log(self.Ns[board_hash] + self.EPS, 2))
